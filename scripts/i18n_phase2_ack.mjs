#!/usr/bin/env node
/**
 * Phase 2: Hash-Manifest + Review-Ack (EN required)
 *
 * Purpose:
 * - For keys with critical DE changes (vs. base), once EN has been updated (Phase 1 condition),
 *   require an explicit review-ack recorded in frontend/src/locales/.i18n_ack/en_ack.json.
 * - The ack captures current DE and EN content hashes for that key as proof it was reviewed.
 * - Fails CI if any required ack is missing or mismatched.
 *
 * Output:
 * - content_backups/ack_required_en.txt listing keys that require ack.
 *
 * Exit codes:
 * - 0 OK (all required acks present and matching)
 * - 1 Missing/mismatched acks
 */
...
function flatten(obj, prefix = '') {
  ...
  for (const [k, v] of Object.entries(obj)) {
    ...
    if (v && typeof v === 'object' && !Array.isArray(v)) Object.assign(res, flatten(v, key));
    else res[key] = v;
  }
  return res;
}
function getLangMergedWorking(lang) {
  ...
  let merged = {};
  try { if (fs.existsSync(rootFile)) merged = deepMerge(merged, JSON.parse(fs.readFileSync(rootFile, 'utf8'))); } catch {}
...
function main() {
  ...
  for (const k of changedKeys) {
    ...
    if (a !== b) criticalKeys.push(k);
  }

  if (!criticalKeys.length) {
    // nothing to ack
    if (!fs.existsSync(REPORT_DIR)) fs.mkdirSync(REPORT_DIR, { recursive: true });
    fs.writeFileSync(path.join(REPORT_DIR, 'ack_required_en.txt'), 'No acks required.\n');
    console.log('[CI:i18n:ack] No critical DE changes; no acks required.');
    process.exit(0);
  }

  // Limit to keys where EN was updated (Phase 1 precondition)
  const keysRequiringAck = criticalKeys.filter(k => (k in enHead) && String(enHead[k]) !== String(enBase[k]));

  const ack = fs.existsSync(EN_ACK_FILE) ? JSON.parse(fs.readFileSync(EN_ACK_FILE, 'utf8')) : { keys: {} };
  ack.keys = ack.keys || {};

  const missing = [];
  for (const k of keysRequiringAck) {
    const entry = ack.keys[k];
    const cur = { de_hash: sha256(deHead[k]), en_hash: sha256(enHead[k]) };
    if (!entry || entry.de_hash !== cur.de_hash || entry.en_hash !== cur.en_hash) {
      missing.push(k);
    }
  }

  if (!fs.existsSync(REPORT_DIR)) fs.mkdirSync(REPORT_DIR, { recursive: true });
  const report = path.join(REPORT_DIR, 'ack_required_en.txt');
  fs.writeFileSync(report, missing.length ? `Ack required for keys (EN review):\n` + missing.map(k => `- ${k}`).join('\n') + '\n' : 'No acks required.\n');

  if (missing.length) {
    console.error(`[CI:i18n:ack] Missing EN acks for ${missing.length} key(s). See ${report}`);
    process.exit(1);
  }

  console.log('[CI:i18n:ack] All required EN acks present and matching.');
}

if (import.meta.url === `file://${process.argv[1]}`) {
  try { main(); } catch (e) { console.error(`[CI:i18n:ack] Error: ${e?.message || e}`); process.exit(1); }
}
